...
exports.createOrder = async (req, res) => {
  try {
    ...
    const {
      ...
      orderNote, scheduledDelivery, paymentMethod, products, total
    } = req.body;
    ...
    const invoiceHtml = `<h2>Order Invoice</h2>
      <p>Name: ${firstName} ${lastName}</p>
      <p>Email: ${email}</p>
      <p>Phone: ${phone}</p>
      <p>Address: ${country}, ${city}, ${state}, ${postcode}</p>
      <p>Company: ${companyName || ''}</p>
      <p>Order Note: ${orderNote || ''}</p>
      <p>Scheduled Delivery: ${scheduledDelivery?.date || ''} ${scheduledDelivery?.note || ''}</p>
      <p>Payment Method: ${paymentMethod}</p>
      <p>Total: $${total}</p>
      <h3>Products</h3>
      <ul>${products.map(p => `<li>${p.name} x${p.quantity} - $${p.price}</li>`).join('')}</ul>`;
    // Send to admin
    await sendEmail({
